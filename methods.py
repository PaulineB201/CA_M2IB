"""
This code is based on M2IB https://github.com/YingWANGG/M2IB/tree/main, IBA https://github.com/bazingagin/IBA, and IBA https://github.com/BioroboticsLab/IBA
"""
import sys
sys.path.append('.')
sys.path.append('../..')
sys.path.append('../.')
from ca_iba import CA_IBA, Estimator
import numpy as np
import clip
import copy
import torch 
...
def text_heatmap_ca_iba(text_t, image_t, model, layer_idx, beta, var, lr=1, train_steps=10, progbar=True):
    ...
    reader = CA_IBA(model, compression_estimator, beta=beta, lr=lr, steps=train_steps, progbar=progbar)
    return reader.text_heatmap(text_t, image_t)

def vision_heatmap_ca_iba(text_t, image_t, model, layer_idx, beta, var, lr=1, train_steps=10, progbar=True):
